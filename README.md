...
* `redis_version`: [default: `4.0.10`]: Redis version to install (only applicable to `src` installs)
...
* `redis_tcp_backlog`: [default: `511`]: TCP `listen()` backlog. In high requests-per-second environments you need a high backlog in order to avoid slow clients connections issues
...
* `redis_daemonize`: [default: `true`]: Whether to run as a daemon
...
* `redis_syslog_enabled`: [default: `false`]: Whether to enable logging to the system logger
...
* `redis_always_show_logo`: [default: `true`]: Force the pre-4.0 behavior and always show an ASCII art logo in startup logs (`>= 4.0.0` only)
...
* `redis_stop_writes_on_bgsave_error`: [default: `true`]: Whether to stop writes on bgsave error
* `redis_rdbcompression`: [default: `true`]: Whether to compress string objects using `LZF` when dump `.rdb` databases
* `redis_rdbchecksum`: [default: `true`]: Whether to checksum (`CRC64`) databases
...
* `redis_slave_serve_stale_data`: [default: `true`]: Whether serve stale data (when a slave loses its connection with the master, or when the replication is still in progress)
* `redis_slave_read_only`: [default: `true`]: Whether a slave instance should accept writes
* `redis_repl_diskless_sync`: [default: `false`]: Whether to use diskless sync
...
* `redis_repl_disable_tcp_nodelay`: [default: `false`]: Whether to disable `TCP_NODELAY` on the slave socket after `SYNC`
...
* `redis_appendonly`: [default: `false`]: Whether to use [Append Only File](http://redis.io/topics/persistence)
...
* `redis_no_appendfsync_on_rewrite`: [default: `false`]: Whether to disable `fsync` during AOF log rewrites
...
* `redis_client_output_buffer_limits.{n}.hard_limit`: [required]: A hard limit, forces an immediate disconnect (e.g. `256mb`)
...
* `redis_vm_overcommit_memory`: [default: `false`]: Whether to set system overcommit policy, [see](http://redis.io/topics/faq#background-saving-is-failing-with-a-fork-error-under-linux-even-if-i39ve-a-lot-of-free-ram). Setting it to `1` is probably what you want
* `redis_transparent_hugepage`: [default: `false`]: Whether to disable transparent huge pages, [see](http://redis.io/topics/latency#redis-latency-problems-troubleshooting). Setting it to `never` is probably what you want
